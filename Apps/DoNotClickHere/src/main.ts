...
export default class MyDashboardElement extends UmbElementMixin(HTMLElement) {
  ...
  constructor() {
    ...
    if (this.shadowRoot) {
      ...
      $(this.shadowRoot).find("#clickMe").on("click", () => {
      ...
      this.consumeContext(UMB_CURRENT_USER_CONTEXT, (instance) => {
        instance.currentUser.subscribe((user) => {
          if (this.shadowRoot && user) {
            const uuibox = $(this.shadowRoot).find('uui-box');
            const currentAttr = uuibox.attr('headline');
            uuibox.attr('headline', currentAttr!.replace('[#name#]', user.name));	
          }
        });
      });
    }
...
customElements.define("do-not-click-here", MyDashboardElement);